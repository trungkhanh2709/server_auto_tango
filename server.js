...
async function waitAndClick(page, selectorOrXpath, isXpath = false, retries = 5, log) {
  for (let i = 0; i < retries; i++) {
    try {
      ...
      log(`DEBUG: waitAndClick attempt ${i+1} failed for ${selectorOrXpath}: ${err.message}`);
    }
    await sleep(300);
  }
...
app.get("/run-tango-sse", async (req, res) => {
  ...
  try {
    ...
    const [newPage] = await Promise.all([
      ...
      page.evaluate(() => {
        const openLink = Array.from(document.querySelectorAll("a")).find(a => a.innerText.trim().toLowerCase() === "open");
    ...
    for (const step of steps) {
      ...
      if (stepText.startsWith("Click on")) {
        ...
        const clickedInput = await waitAndClick(newPage, inputSelector, false, 5, log);
        if (clickedInput) {
          ...
          const clickedBtn = await waitAndClick(newPage, btnXpath, true, 5, log);
        ...
        if (match) {
          ...
          let typed = false;
          if (lastClickedSelector) typed = await typeReactInput(newPage, lastClickedSelector, textToType, log);
          if (!typed) {
